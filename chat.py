...
def id():
	...
	try:
		...
		if len(IdTeman) != 0:
			print warna.hijau + '\nBerhasil mengumpulkan' + warna.kuning + len(IdTeman) + warna.hijau + " ID Teman"
		else:
			sys.exit()
	while 1:
		id_terkumpul(membuka(next))
		print warna.biru + len(IdTeman) + warna.hijau + ' ID terambil'
		sys.stdout.flush()
		try:
			next = br.find_link(url_regex='friends_center_main').url
		except:
			print warna.kuning + '\nBerhasil mengumpulkan ' + warna.merah + len(IdTeman) + warna.kuning + ' ID Teman'
			break
	if len(IdTeman) != 0:
